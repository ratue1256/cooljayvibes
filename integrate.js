...
// ==== 1. Update Configurator ====
let configHtml = fs.readFileSync('configurator.html', 'utf8');

// Insert script ref
if (!configHtml.includes('recommendation_engine.js')) {
    configHtml = configHtml.replace('<script src="lang.js"></script>', '<script src="recommendation_engine.js"></script>\n<script src="lang.js"></script>');
}

// Full script replacer
const startMarker = '<script>';
const endMarker = '</script>\n<script src="lang.js"></script>';

// Because the original has `<script>` opening without ID and then closes matching the pattern above
// Wait, looking at lines 329-331:
// 329:     </script>
// 330: <script src="lang.js"></script>

const newScript = `    <script id="global-scripts">
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function loadStep(stepIndex) {
                ...
                stepData.options.forEach((opt, idx) => {
                    ...
                    btn.innerHTML = \`
                        <span class="material-symbols-outlined text-4xl mb-4 group-hover:text-primary transition-colors text-white">\${opt.icon}</span>
                        <span class="font-semibold text-center">\${opt.text}</span>
                    \`;
            ...
            nextBtn.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    ...
                    if (currentStep < steps.length) {
                        ...
                        setTimeout(() => {
                            let result = window.calculateBestDevice ? window.calculateBestDevice(userPrefs) : { winner: "Pixel 10 Pro XL", reason: "Native AI", runnerUp: "S25 Ultra" };
                            
                            cardContainer.classList.remove('p-10');
                            cardContainer.classList.add('p-0');

                            cardContainer.innerHTML = \`
                                <div class="bg-[#2c2c2e]/50 border-b border-white/5 p-8 text-center bg-gradient-to-b from-primary/10 to-transparent">
...
let scriptStartIdx = configHtml.indexOf('<script>');
let scriptEndIdx = configHtml.indexOf('</script>\n<script src="lang.js"></script>');

if (scriptStartIdx !== -1 && scriptEndIdx !== -1) {
    configHtml = configHtml.slice(0, scriptStartIdx) + newScript + configHtml.slice(scriptEndIdx + '</script>'.length);
}
fs.writeFileSync('configurator.html', configHtml);


// ==== 2. Update Guides ====
let guidesHtml = fs.readFileSync('guides.html', 'utf8');

// Replace Apple text with Logo SVG
guidesHtml = guidesHtml.replace(/<span class=\"material-symbols-outlined text-4xl mb-6 text-white\">apple<\/span>/g, '<svg class="w-9 h-9 mb-6 text-white fill-current" viewBox="0 0 384 512" xmlns="http://www.w3.org/2000/svg"><path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.3 48.6-.6 88.5-84.6 100.8-121.2-46.7-20.9-59.5-68.5-59.9-89.4zM258 87.7c26.9-32.9 23.3-71.1 22.4-74.5-27.1 1.4-60.6 18.2-78.1 39.3-15.3 18.2-22.9 44.6-21.7 69.4 30.6 2.3 58-15.5 77.4-34.2z"/></svg>');

// Make guides interactive by expanding when clicked to show a "real source" step-by-step
const targetGuideHtml = `
            <div class="w-full max-w-3xl grid grid-cols-1 gap-6 text-left" id="guides-container">
                <div
                    ...
                    <div id="guide-android-to-ios" class="hidden mt-4 pt-4 border-t border-white/10 text-white/80 space-y-4 text-sm animate-fade-in">
                        ...
                        <button class="bg-emerald-500/20 text-emerald-400 px-4 py-2 rounded-full font-semibold mt-2 hover:bg-emerald-500/30 transition-colors">View Deep Dive Article</button>
                    </div>
                </div>
                
                <div class="apple-card rounded-[2rem] p-8 flex flex-col group cursor-pointer hover:border-gray-500/50" onclick="toggleGuide('ios-to-android')">
                    ...
                    <div id="guide-ios-to-android" class="hidden mt-4 pt-4 border-t border-white/10 text-white/80 space-y-4 text-sm animate-fade-in">
                        ...
                        <button class="bg-gray-500/20 text-gray-300 px-4 py-2 rounded-full font-semibold mt-2 hover:bg-gray-500/30 transition-colors">View Deep Dive Article</button>
                    </div>
                </div>
            </div>
`;

// Replace the grid container in guides
// Be safe with regex match of the main container div contents
const mainRegex = /<div class=\"w-full max-w-3xl grid grid-cols-1 md:grid-cols-2 gap-6 text-left\">[\s\S]*?<\/div>\s*<\/main>/;
guidesHtml = guidesHtml.replace(mainRegex, targetGuideHtml + '\n        </main>');
...
\`;
if (!guidesHtml.includes('function toggleGuide')) {
    guidesHtml = guidesHtml.replace('</body>', toggleScript + '\n</body>');
}

fs.writeFileSync('guides.html', guidesHtml);
